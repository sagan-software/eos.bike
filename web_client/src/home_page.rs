use crate::scatter::{EosJsConfig, Network, RequiredFields, Scatter, Transaction};
use crate::Context;
use contract::Slug;
use eosio::*;
...
impl HomePage<'_> {
    pub fn render(state: &Context) -> Result<Element, JsValue> {
        let el = state.document.create_element("div")?;
        el.set_class_name("home");

        let logo = render_logo(state)?;
        el.append_child(logo.as_ref())?;

        let form = render_form(state)?;
...
fn render_form(state: &Context) -> Result<Element, JsValue> {
    ...
    let cb = Closure::wrap(Box::new(move |e: Event| {
...

        {
            ...
            let f = Scatter::connect("eos.bike")
                .and_then(|scatter| {
            ...
            let b = Closure::wrap(Box::new(move |connected| {
                web_sys::console::log_2(&JsValue::from_str("!!!"), &connected);
            }) as Box<dyn FnMut(JsValue)>);
            future_to_promise(f).then(&b);
            b.forget();
        }
    }) as Box<dyn FnMut(_)>);
